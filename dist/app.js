(() => {
  ...
  function showPopover({ message, actions, onClose, anchorElement }) {
  ...
  // pill.component.js
  var VALID_VARIANTS = ["signed-in", "signed-out"];
  var VALID_SIZES3 = ["xs", "sm", "md", "lg", "xl"];
  var PILL_SIZE_CLASSES = {
    xs: "pill--xs",
    sm: "pill--sm",
    md: "pill--md",
    lg: "pill--lg",
  ...
  function createPill({
    ...
    const variantClass = VALID_VARIANTS.includes(variant) ? variant : "signed-out";
    const sizeClass = VALID_SIZES3.includes(size) ? PILL_SIZE_CLASSES[size] : "pill--md";
...
This typically indicates that your device does not have a healthy Internet connection at the moment. The client will operate in offline mode until it is able to successfully connect to the backend.`;
  ...
  async function handleLoginClick() {
    try {
      ...
      if (window.showToast) {
        window.showToast(
          err.code === "auth/popup-closed-by-user" ? "Inicio de sesi\xF3n cancelado" : "Error al autenticar con Google"
        );
      }
    }
  }
  ...
  async function backupToFirestore() {
    const user = auth.currentUser;
    if (!user) {
      if (window.showToast) window.showToast("Debes iniciar sesi\xF3n primero.");
      return;
    }
  ...
  async function restoreFromFirestore(silent = false) {
    const user = auth.currentUser;
    if (!user) {
      if (!silent && window.showToast) window.showToast("Debes iniciar sesi\xF3n primero.");
      return;
    }
  ...
  function formatAgoFromLastSync(lastSync) {
    ...
    return `hace ${Math.floor(diffMins / 1440)} d\xEDas`;
  }
  async function renderFirebaseBackupInfo(container) {
    ...
    try {
      ...
      if (snap.exists() && snap.data().lastSync) {
        let updateAgo = function() {
          ...
          el.textContent = `\xDAltima sincronizaci\xF3n: ${lastSyncStr} (${ago})`;
          const diffMs = /* @__PURE__ */ new Date() - lastSync;
          if (diffMs >= 6e4 && el._lastSyncInterval) {
            clearInterval(el._lastSyncInterval);
            el._lastSyncInterval = null;
          }
        };
        const lastSync = snap.data().lastSync?.toDate?.() || /* @__PURE__ */ new Date();
        const lastSyncStr = lastSync.toLocaleString();
        updateAgo();
        if (/* @__PURE__ */ new Date() - lastSync < 6e4) {
          el._lastSyncInterval = setInterval(updateAgo, 1e3);
        }
      } else {
        el.textContent = "A\xFAn no has sincronizado.";
      }
    } catch (err) {
  ...
  function getNextOccurrence(baseDateInput, frequency, afterDate = null) {
    const baseDate = new Date(baseDateInput.getTime());
    if (frequency === "none") {
      if (afterDate && baseDate.getTime() <= new Date(afterDate.getTime()).getTime()) {
        return null;
      }
      return new Date(baseDate.getTime());
    }
    const comparisonDate = afterDate ? new Date(afterDate.getTime()) : new Date((/* @__PURE__ */ new Date()).getTime() - 1);
    let currentOccurrence = new Date(baseDate.getTime());
    while (currentOccurrence.getTime() <= comparisonDate.getTime()) {
      const next = advanceDateByFrequency(currentOccurrence, frequency, baseDate);
  ...
  function advanceDateByFrequency(currentDate, frequency, originalStartDate) {
  ...
  // core/counterManager.js
  var _getConfig = () => ({});
  var _getFilterTags = () => [];
  var _openCounterModal = () => {
  };
  var _renderFilterTags = () => {
  };
  var _getDomListElement = () => null;
  var _getCounterTimeElement = () => null;
  var _renderCounters = () => {
  };
  var _showDeleteConfirm = (opts) => {
    Promise.resolve().then(() => (init_popover(), popover_exports)).then(({ showPopover: showPopover2 }) => showPopover2(opts));
  };
  var _onAfterDelete = () => {
  };
  function initCounterManager(dependencies) {
  ...
  function deleteCounter(id, occurrenceDateString) {
    ...
    if (occurrenceDateString && counter.frequency !== "none") {
      ...
      const occurrenceISOString = occurrenceDate.toISOString().split("T")[0];
  ...
  function addOrUpdateCounter(counterData, editIndex) {
    ...
    const newCounter = {
      ...
      id: editIndex !== null && counters[editIndex] ? counters[editIndex].id : Date.now().toString(),
      originalDate: frequency !== "none" ? editIndex !== null && counters[editIndex] && counters[editIndex].originalDate ? counters[editIndex].originalDate : date : null,
      history: editIndex !== null && counters[editIndex] ? counters[editIndex].history : [],
      endDate: frequency !== "none" ? endDate : void 0
      // <-- Asegúrate de guardar endDate
    };
    if (editIndex !== null) {
      const oldCounter = counters[editIndex];
      if (oldCounter.frequency && oldCounter.frequency !== "none" && oldCounter.date !== newCounter.date) {
      }
  ...
  function renderCounters() {
    ...
    if (filterTags.length) {
      filtered = counters.filter(
        (c) => Array.isArray(c.tags) && filterTags.every((tag) => c.tags.includes(tag))
      );
    }
    filtered.forEach((counter, idxInFilteredArray) => {
      ...
      if (isRecurring) {
        ...
        let currentOccurrenceDate = getNextOccurrence(
          originalStartDate,
          counter.frequency
        );
        if (currentOccurrenceDate) {
          ...
          if (now < currentOccurrenceDate) {
            ...
            text = `Pas\xF3 hace ${formatDiff(diff, config)}`;
          }
          ...
          if (counter.frequency === "annual") {
            const day = currentOccurrenceDate.getDate().toString().padStart(2, "0");
            ...
            window.createButton({
              ...
              onClick: (e) => {
                if (isRecurring) {
                  ...
                  const occurrenceDate = li2 ? li2.getAttribute("data-occurrence-date") : null;
                  _showDeleteConfirm({
                    message: "\xBFQu\xE9 deseas borrar?",
                    actions: [
                      {
                  ...
                  _showDeleteConfirm({
                    message: "\xBFEst\xE1s seguro de eliminar este contador?",
                    actions: [
          ...
          repetitionsRendered++;
          occurrenceIdxForId++;
          currentOccurrenceDate = advanceDateByFrequency(
            new Date(currentOccurrenceDate.getTime()),
            counter.frequency,
            originalStartDate
          );
        }
        while (currentOccurrenceDate && repetitionsRendered < MAX_REPETITIONS_TO_RENDER) {
    ...
    counters.forEach((counter, originalIdx) => {
      if (counter.frequency && counter.frequency !== "none") {
        ...
        while (occurrenceRenderIdx < MAX_REPETITIONS_TO_CHECK) {
          ...
          const globalEndDate = counter.endDate ? new Date(counter.endDate) : null;
          if (globalEndDate) globalEndDate.setHours(23, 59, 59, 999);
          if (globalEndDate && now > globalEndDate && occurrenceDate > globalEndDate) {
            if (!timeSpan.textContent.includes("Finalizado")) {
              ...
              if (nameSpan && !nameSpan.textContent.includes("(Finalizado)")) {
                ...
                if (dateSpan)
                  dateSpan.textContent = `(Termin\xF3 el ${globalEndDate.toLocaleDateString(
                    "es-ES"
                  )})`;
              }
            }
        ...
        const timeSpan = _getCounterTimeElement(originalIdx, null);
        if (timeSpan) {
  ...
  var BottomSheet = class {
    ...
    _scheduleViewportResizeOnFocus() {
      ...
      this._focusResizeTimeouts = delays.map(
        (d) => setTimeout(() => this._onVisualViewportResize(), d)
      );
    }
    ...
    _setupGrabberDrag(grabber) {
      ...
      const onEnd = () => {
        ...
        const distanceThreshold = this.sheet.offsetHeight * 0.5;
        const velocityThreshold = 0.4;
        const minDragForVelocity = 50;
        const shouldClose = deltaY > distanceThreshold || velocity > velocityThreshold && deltaY > minDragForVelocity;
  ...
  function showDeleteConfirm({ message, actions, anchorElement, counterName }) {
    ...
    if (isMobile2) {
      ...
      const cancelHtml = cancelBtn ? `<button type="button" class="${cancelBtn.className || ""}" data-delete-action="cancel">${cancelBtn.text}</button>` : "";
      const confirmHtml = confirmBtns.map(
        (a, i) => `<button type="button" class="${a.className || ""}" data-delete-action="confirm-${i}">${a.text}</button>`
      ).join("");
      const sheet = new bottom_sheet_component_default({
        header: '<span class="modal-title">\xBFEst\xE1s seguro?</span>',
...
<div id="undo-toast" class="toast" style="display:none;">
  ...
  var addBtnContainer = document.getElementById("add-button-container");
  if (addBtnContainer) {
    const addBtn = window.createButton({
      text: "A\xF1adir contador",
      icon: "plus",
      color: "add",
      id: "open-add-modal",
  ...
  var counterModalRoot = null;
  var counterSheetView = null;
  ...
  function openCounterModal(mode = "new", idx = null) {
    ...
    if (isMobile2) {
      ...
      const view = new bottom_sheet_component_default({
        header: headerEl,
        body: bodyEl,
        footer: footerEl,
        closable: true,
  ...
  function renderTagSuggestions() {
    ...
    allTags.forEach((tag) => {
      ...
      tagEl.addEventListener && tagEl.addEventListener("click", () => {
        modalTags.push(tag);
        renderTagsList();
        renderTagSuggestions();
      });
  ...
  document.getElementById("counter-form-modal").addEventListener("submit", async function(e) {
    e.preventDefault();
    const name4 = document.getElementById("modal-counter-name").value.trim();
    const date = document.getElementById("modal-counter-date").value;
    const time = document.getElementById("modal-counter-time").value;
    const frequency = document.getElementById("modal-counter-frequency").value;
    const endDate = document.getElementById("modal-counter-end-date").value;
    if (!name4 || !date) return;
    if (frequency !== "none" && endDate && new Date(endDate) < new Date(date)) {
      alert(
        "La fecha de finalizaci\xF3n no puede ser anterior a la fecha de inicio del contador."
      );
      return;
    }
    ...
    const counterData = {
      ...
      tags: [...modalTags],
      frequency,
      endDate: frequency !== "none" ? endDate : ""
      // Save endDate only if recurring
    };
  ...
  function renderAuthStatusIndicator() {
    ...
    const text = loading ? "Cargando\u2026" : signedIn ? user?.displayName || user?.email || "Conectado" : "Iniciar sesi\xF3n";
    const pill = window.createPill({
      ...
      size: "md",
      ariaLabel: loading ? "Comprobando sesi\xF3n\u2026" : `Estado de sesi\xF3n: ${text}`,
      title: text
    });
    ...
    const options = signedIn ? [
      { id: "config", label: "Configuraci\xF3n", icon: "settings", action: () => openConfigModal() },
      { id: "logout", label: "Cerrar sesi\xF3n", icon: "log-out", action: () => handleLogoutClick() }
    ] : [
      { id: "config", label: "Configuraci\xF3n", icon: "settings", action: () => openConfigModal() },
      { id: "login", label: "Iniciar sesi\xF3n", icon: "log-in", action: () => handleLoginClick() }
    ];
    options.forEach((opt) => {
      const btn = document.createElement("button");
      btn.type = "button";
    ...
    pill.onclick = (e) => {
      ...
      if (isMobile()) {
        showOptionsBottomSheet({
          ...
          title: text,
          onSelect: (val, opt) => {
            const selected = options.find((x2) => x2.id === val);
          ...
          onClick: async function() {
            if (deleteIdx !== null) {
              ...
              showToast("Contador eliminado", async function() {
                ...
                if (lastDeleted) {
                  counters2.splice(lastDeleted.idx, 0, { ...lastDeleted });
                  saveCounters(counters2);
                  renderCounters();
                  lastDeleted = null;
                  if (isSignedIn()) {
                    try {
  ...
  `;
    ...
    <div class="units-section-modal units-section-box" style="margin-top: 10px">
      ...
      <div id="config-form-modal" class="units-buttons-row">
        <button type="button" class="unit-btn" data-unit="years">A\xF1os</button>
        <button type="button" class="unit-btn" data-unit="months">Meses</button>
        <button type="button" class="unit-btn" data-unit="days">D\xEDas</button>
    ...
    <div id="firebase-section" class="units-section-modal units-section-box" style="margin-top: 22px">
      <div class="units-section-title">
        <i data-lucide="log-in" style="margin-right: 7px"></i><span id="account-section-title">Iniciar sesi\xF3n</span>
      </div>
  ...
  `;
    ...
    if (configFormModal) {
      let syncButtonsFromConfig = function(cfg) {
    ...
    function renderFirebaseAuthArea() {
      ...
      if (!area) return;
      if (titleEl) titleEl.textContent = isSignedIn() ? "Tu cuenta" : "Iniciar sesi\xF3n";
      if (sectionTitle) {
      ...
      if (isSignedIn()) {
        ...
        logoutBtn.innerHTML = '<i data-lucide="log-out"></i> Cerrar sesi\xF3n';
        ...
        loginBtn.innerHTML = '<i data-lucide="log-in"></i> Iniciar sesi\xF3n con Google';
    ...
    if (exportBtn) {
      exportBtn.onclick = function() {
        ...
        if (!XLSXlib) {
          alert("No se ha cargado la librer\xEDa XLSX.");
          return;
        }
        const counters = getCounters();
    ...
    if (importInput) {
      importInput.addEventListener("change", function(e) {
        ...
        if (!file) {
          window.showToast && window.showToast("No se seleccion\xF3 ning\xFAn archivo.");
          return;
        }
        ...
        reader.onload = function(evt) {
          try {
            ...
            if (!Array.isArray(json) || !json.length) {
              window.showToast && window.showToast("El archivo Excel est\xE1 vac\xEDo o no tiene datos.");
              return;
            }
            ...
            const counters = json.map((row) => ({
              name: String(row.nombre || "").trim(),
              date: String(row.fecha || "").trim(),
              tags: typeof row.tags === "string" && row.tags.trim() ? row.tags.split(/,\s*/) : []
            })).filter((c) => c.name && c.date);
            if (!counters.length) {
              window.showToast && window.showToast(
                "No se encontraron contadores v\xE1lidos en el archivo."
              );
              return;
            }
            localStorage.setItem("counters", JSON.stringify(counters));
            renderCounters();
            window.showToast && window.showToast("Contadores importados correctamente");
            view.close();
          } catch (err) {
            window.showToast && window.showToast(
              "Error al importar el archivo. \xBFEs un Excel v\xE1lido?"
            );
          }
        };
